$uuid = [guid]::NewGuid().ToString()
$headers = @{
    'x-api-key' = 'sdgdskldFPLGfjHn1421dgnlxdGTbngdflg6290bRjslfihsjhSDsdgGHH25hjf'
    'x-user-key' = '__'
    'x-request-id' = $uuid
}
$body = @{
    'InstrumentId' = 7991
    'UnitsToDeduct' = $null
} | ConvertTo-Json
$response = Invoke-WebRequest -Uri 'https://public-api.etoro.com/api/v1/trading/execution/market-close-orders/positions/3377763784' -Headers $headers -Method Post -Body $body -ContentType 'application/json'
$response.Content